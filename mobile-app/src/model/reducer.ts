...
export function reduceAppState(state: AppState, event: AppEvent): AppState {
  switch (event.type) {
    ...
    case "terminal.output-chunk": {
       ...
       * skaitė `lines[lines.length - 1]` template'e. Su `noUncheckedIndexedAccess` tas skaitymas
       * yra `string | undefined`, ir klaidos atveju į buferį nutekėtų eilutė „undefined…".
       * Tuščias masyvas ir yra vienintelis atvejis, kai to indekso nėra, tad sąlyga imama iš
       * paties indekso: dvi šakos lieka tos pačios, bet invariantą dabar tikrina tipų sistema.
       */
