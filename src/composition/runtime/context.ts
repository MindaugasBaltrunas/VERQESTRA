 ...
 * Vedama iš šio modulio vietos: `<paketas>/dist/composition/runtime/context.js` → TRYS lygiai
 * aukštyn. Skaičius seka šio failo gylį, tad kompoziciją perstruktūrizavus (2026-08-23,
 * `composition/*.ts` → pakatalogiai) jį reikėjo pakelti nuo dviejų iki trijų. Klaida čia yra tyli:
 * paketo šaknis nurodytų `dist/`, šablonų nerastų, ir `install` bei benchmark celės kristų
 * fail-closed — būtent tai ir pagavo `composition-loop-cell-provisioning` testas.
 */
