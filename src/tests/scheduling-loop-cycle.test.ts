...
function world(options: {
  ...
  slotOutcomes?: Record<string, SlotChildOutcome>;
  withdrawn?: string[];
  ...
  const ports: LoopCyclePorts = {
    ...
    runSlotTask: (slot) => {
      ...
      return Promise.resolve(outcome ?? ((options.runOk ?? true) ? { status: "succeeded" } : { status: "task-failed" }));
    },
  };
...
test("infra slot baigtis: naujų slot'ų nebeprovisioninama, o loop'as baigiasi TUO PAČIU infra kodu", async () => {
  ...
  const w = world({
    ...
    slotOutcomes: { "0001": { status: "infrastructure", code: USAGE_LIMIT_EXIT_CODE } },
  });
  w.ports.scheduler.recordOutcome = (taskId, outcome) => {
  ...
  assert.deepEqual(
    ...
    "apskaita gauna baigtį NEPAKEISTĄ — būtent iš jos integracija sprendžia neparkuoti",
  );
...
test("infra baigtis LAIKO papildymą: atsilaisvinęs slot'as naujo darbo negauna", async () => {
  ...
  w.ports.runSlotTask = async (slot) => {
    ...
    if (slot.worker_id === "w1") return { status: "infrastructure", code: USAGE_LIMIT_EXIT_CODE };
...
test("ne-infra ne-nulis baigtis eilės degimo NEKEIČIA", async () => {
  ...
  const w = world({
    selections: [taskSelection(), taskSelection({ taskId: "0002" })],
    slotOutcomes: { "0001": { status: "task-failed", code: 1 }, "0002": { status: "succeeded" } },
  });
  const outcome = await runLoopCycle(w.ports);
