...
// Policy-proposal resolution (approve/reject/apply) surfaced into the shipped
// dashboard tree. This panel is mounted by `DashboardPage`; it is the single
// live home of the resolution UI. It intentionally owns its own proposal state
// (fetch on mount + after every decision) because `useDashboardController` only
// carries the propose-only flow and is out of scope for this change.

type ProposalVerb = "approve" | "reject" | "apply";
    ...
    <section className="panel proposal-inbox" aria-labelledby="policy-proposals-title">
      ...
      {proposals && visibleProposals.length === 0 ? (
        <div className="inbox-zero">
          ...
          <p>{t(view === "active" ? "New proposals will appear here with a complete audit trail." : "Applied and rejected changes will appear here.")}</p>
        </div>
      ) : null}
      {visibleProposals.map((resolved) => (
        <PolicyProposalCard
          key={`${resolved.proposal.policy_file}/${resolved.proposal.setting_id}/${resolved.proposal.timestamp}`}
          resolved={resolved}
          onDecide={decide}
        />
      ))}
